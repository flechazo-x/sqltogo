...
func (t *DefaultTemplate) Execute(data interface{}) (*bytes.Buffer, error) {
	...
	if err != nil {
		return nil, errors.New(fmt.Sprintf("template parse error:%v", t.text))
	}

	buf := new(bytes.Buffer)
	if err = tem.Execute(buf, data); err != nil {
		return nil, errors.New(fmt.Sprintf("template execute error:%v", t.text))
	}
	...
	if err != nil {
		return nil, errors.New(fmt.Sprintf("go format error::%v", t.text))
	}
