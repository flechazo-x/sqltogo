...
import (
	...
	"sort"
	"strings"
	"work/sqltogo/ddl-parser/console"
	"work/sqltogo/ddl-parser/parser"
	"work/sqltogo/sql/collection"
	"work/sqltogo/sql/converter"
	"work/sqltogo/sql/model"
...
func Parse(filename, database string, strict bool) ([]*Table, error) {
	p := parser.NewParser()         //解析器
	tables, err := p.From(filename) //解析表结构
	if err != nil {
		return nil, err
	}

	nameOriginals := parseNameOriginal(tables)      //表名
	indexNameGen := func(column ...string) string { //生成名字
	...
	for indexTable, e := range tables {
		var (
			...
			uniqueKeyMap     = make(map[string][]string) //唯一索引
			normalKeyMap     = make(map[string][]string) //普通索引
			columns          = e.Columns                 //列
		)
		...
		if column.Constraint != nil {
			if column.Name == primaryColumn {
				if !column.Constraint.AutoIncrement && dataType == "int64" {
					fmt.Printf("[convertColumns]: The primary key %s is recommended to add constraint `AUTO_INCREMENT`\n", column.Name)
				}
			} else if column.Constraint.NotNull && !column.Constraint.HasDefaultValue {
				fmt.Printf("[convertColumns]: The column %s is recommended to add constraint `DEFAULT`\n", column.Name)
			}
		}
